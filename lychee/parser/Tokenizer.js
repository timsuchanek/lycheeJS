...
lychee.define('lychee.Tokenizer').exports(function(lychee, global) {

	var _Error = function(message, token) {
	...
	function is_identifier_character(character) {
		...
		return (
			...
			|| is_digit(character)
			|| is_unicode_connector_punctuation(character)
	...
	function is_unicode_connector_punctuation(character) {
		return _REGEXP.UNICODE.connector_punctuation.test(character);
	};

	function is_unicode_combining_mark(character) {

		return (
			_REGEXP.UNICODE.non_spacing_mark.test(character)
			|| _REGEXP.UNICODE.space_combining_mark.test(character)
		);

	};

	function is_unicode_letter(character) {
		return _REGEXP.UNICODE.letter.test(character);
	};


	var _KEYWORDS = array_to_hash([
		'break',
	...
	var _KEYWORDS_ATOM = array_to_hash([
		'false', 'null', 'true', 'undefined'
	]);


	var _KEYWORDS_BEFORE_EXPRESSION = array_to_hash([
		'return', 'new', 'delete', 'throw', 'else', 'case'
	]);


	var _OPERATORS = array_to_hash([
		'delete',
		'in', 'instanceof',
		'new',
		'typeof',
	...
	var _RESERVED_WORDS = array_to_hash([
		'abstract',
		'boolean', 'byte',
		'char', 'class',
		'double',
	...
	var _CHARS = {
		...
		PUNC_BEFORE_EXPRESSION: array_to_hash([
			'[', '{', '(', ',', '.', ';', ':'
		]),

		WHITESPACE: array_to_hash([
			" ",
			"\n",
			"\r",
			"\t",
	...
	Class.prototype = {
		...
		reset: function(source) {
			...
			this.__source = source.replace(/\r\n?|[\n\u2028\u2029]/g, "\n").replace(/^\uFEFF/, '');

			this.__position = 0;
			this.__line = 0;
			this.__column = 0;
			this.__commentsbefore = [];
			this.__newlinebefore = false;
			this.__regex_allowed = false;
		...
		__next: function(inString) {
			...
			if (character === "\n") {
				...
				this.__newlinebefore = this.__newlinebefore || !inString;
				this.__line++;
				this.__column = 0;
		...
		find: function(what) {
			...
			var position = this.__source.indexOf(what, this.__position);
			if (position === -1) {
				return null;
			}
		...
		token: function(type, value, isComment) {
			...
			var token = {
				type: type,
				value: value,
				line: this.__token.line,
				column: this.__token.column,
				position: this.__token.position,
				endposition: this.__position,
				newlinebefore: this.__newlinebefore
			};
		...
		readNumber: function(prefix) {
			...
			if (_REGEXP.HEX_NUMBER.test(number) === true) {
				number = parseInt(number.substr(2), 16);
			} else if (_REGEXP.OCT_NUMBER.test(number) === true) {
				number = parseInt(number.substr(1), 8);
			} else if (_REGEXP.DEC_NUMBER.test(number) === true) {
			...
			if (!isNaN(number)) {
				...
				return this.token('num', number);

			} else {

				throw new _Error(
					'Invalid Number Syntax: ' + number,
					this.__token
				);
		...
		readString: function() {
			...
			while(true) {
				...
				if (character === "\\") {
					...
					character = this.readWhile(function(character) {
						...
						if (character >= "0" && character <= "7") {
							...
							} else if (first <= "3" && length <= 2) {
								length++;
								return true;
							} else if (first >= "4" && length <= 1) {
								length++;
								return true;
							}
					...
					if (length > 0) {
						character = String.fromCharCode(parseInt(character, 8));
					} else {
						character = this.readEscapedChar(true);
					}
		...
		readLineComment: function() {
			...
			if (index === null) {
				comment = this.__source.substr(this.__position);
			...
			return this.token('comment1', comment, true);
		...
		readMultilineComment: function() {
			...
			if (index === null) {
				...
				var comment = this.__source.substring(this.__position, index);

				this.__position = index + 2;
				this.__line += comment.split("\n").length - 1;
				this.__newlinebefore = comment.indexOf("\n") >= 0;


				return this.token('comment2', comment, true);
		...
		readName: function() {
			...
			var character = null;
			while((character = this.peek()) !== null) {
			...
			if (
				Object.prototype.hasOwnProperty.call(_KEYWORDS, name) === true
				&& escaped === true
			) {
		...
		readRegexp: function(regexp) {
			...
			var mods = this.readName();
			return this.token('regexp', [ regexp, mods ]);
		...
		readOperator: function(prefix) {
			...
			if (this.peek()) {
				...
				if (Object.prototype.hasOwnProperty.call(_OPERATORS, tmp) === true) {
					this.__next();
					operator = tmp;
				}

				tmp = operator + this.peek();
				if (Object.prototype.hasOwnProperty.call(_OPERATORS, tmp) === true) {
					this.__next();
					operator = tmp;
				}
			...
			return this.token('operator', operator);
		...
		readWord: function() {
			...
			if (Object.prototype.hasOwnProperty.call(_OPERATORS, word)) {
				return this.token('operator', word);
			}

			if (Object.prototype.hasOwnProperty.call(_KEYWORDS, word) === false) {

				if (Object.prototype.hasOwnProperty.call(_KEYWORDS_ATOM, word)) {
		...
		next: function() {
			...
			this.__token.line = this.__line;
			this.__token.column = this.__column;
			this.__token.position = this.__position;
