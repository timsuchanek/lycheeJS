...
lychee.define('ui.Radios').tags({
	...
	var Class = function(values, active, callback, scope) {
		...
		this.__wrapper.className = 'ui-radios';
		...
		for (var v = 0, l = values.length; v < l; v++) {
			...
			if (active === value) {
				this.__active = v;
				element.className = 'ui-radio active';
			} else {
				element.className = 'ui-radio';
			}
			...
			this.__wrapper.appendChild(element);
			this.__wrapper.appendChild(label);
			...
			for (var e = 0, el = this.__elements.length; e < el; e++) {
				...
				if (this.__values[e] === value) {
					...
					if (this.__active !== null) {
						...
						this.__elements[this.__active].className = 'ui-radio';
						this.__elements[e].className = 'ui-radio active';
